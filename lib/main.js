          ...
          minify = minify.replace(/(?:background\-image|background|content|border\-image)\:[^;\n]*\)/g, function (rootMatch) {
            ...
            return rootMatch.replace(/url\((?!data:)['"]?([^\)'"]+)['"]?\)/g, function (match, url) {
              ...
              if (options.production) {
                ...
                var fileExt = path.extname(url).substr(1).toLowerCase();
                var mimeTypeSuffix = ( mimeTypeSuffixMap[ fileExt ] || fileExt );
                console.log( 'mimeTypeSuffix', mimeTypeSuffix );
                var imageResource = compile(relativePath.substr(options.publicDir.length + 1), relativePath, S3, options, 'image', 'image/'+mimeTypeSuffix, Date.now(), null, null)();
...
var CDN = function(app, options, callback) {
  ...
  // check for presence of values if operating in production mode
  if (options.production) {
    requiredForProduction.forEach(function(index) {
      if (typeof options[index] === 'undefined') {
        throwError('missing option "' + index + '"');
      }
    });
  ...
  if (options.production && !options.disableWalk) {
    var walker = function () {
      var walker       = walk.walk(options.viewsDir)
        ...
        , hbsRegex     = /{{\s*CDN ((?:['"][^'"]+['"](?:\s)*)*)[^\}]*}}/ig
        , extConfigMap = {
          '.jade': { regexCDN: stdRegex },
          '.ejs': { regexCDN: stdRegex },
          '.hbs': { regexCDN: hbsRegex, matchFn: function (s) {
              s = s.replace(/\s+/g, '');
              if(/['"]['"]/.test(s)) {
                s = '[' + s.replace(/['"]['"]/g, '", "') + ']'
              }
      ...
      walker.on('file', function(root, stat, next) {
        var validExts = options.extensions || Object.keys( extConfigMap );
        ...
        if (_.indexOf(validExts, ext) !== -1) {
          fs.readFile(path.join(root, stat.name), 'utf8', function(err, data) {
            if (err) throwError(err);
            var extConfig = extConfigMap[ ext ];
            var match;
            var matchResult;
            var regexCDN = extConfig.regexCDN;
            while( (match = regexCDN.exec(data)) ) {
              if (extConfig.matchFn) {
                matchResult = extConfig.matchFn(match[1]);
              } else {
                matchResult = match[1];
              }
              if ( matchResult ) results.push(matchResult);
            }
            next();
          });
        } else {
